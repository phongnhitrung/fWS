...
echo'
	...
	<div class="row-fluid top-buffer col-md-12 col-lg-12" >
			...
			<div id="motiongallery" style="position:absolute;left:0;top:0;white-space: nowrap;">	
				<div class="sai_blog_script_holder">
					<table id="trueContainer" width="100%" border="0" cellspacing="8">
						...
						foreach($a_apps as $k => $v){
							echo '
							<td valign="middle"><a href="'.$globals[ind].'act=a_apps&app='.$k.'">
		...
		//Check For Update, Skip update for default Apps.
		foreach($ampps_apps_installed as $j => $m){
			foreach($a_apps as $k => $v){
				if($m['aid'] == $v['aid'] && $m['version'] < $v['version']){
					$chkupdate[$i]['aid'] = $v['aid'];
					$chkupdate[$i]['name'] = $v['fullname'];
					$i++;
				}
			}
		}
			...
			echo '<div class="col-md-12 col-lg-12"><!----#Update---->
					<div class="alert alert-info" >
						<a href = "#" class="close" data-dismiss="alert">&times;</a>
							<center><span class="sai_exp2"><img src="'.$theme['images'].'upgrade_logo.png" height="25" width="25" /> &nbsp'.$l[update_note];
							foreach($chkupdate as $k => $v){
							echo'
							<a href="'.$globals['ind'].'act=a_apps_update&app='.$chkupdate[$k]['aid'].'" alt="" style="text-decoration:none;"><u>'.$chkupdate[$k]['name'].'</u>,&nbsp</a>';
							}
					echo' </span></center>
					</div>
				</div>';
